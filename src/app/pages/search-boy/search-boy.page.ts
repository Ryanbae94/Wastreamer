...
@Component({
  selector: 'app-search-boy',
  templateUrl: './search-boy.page.html',
  styleUrls: ['./search-boy.page.scss'],
})
export class SearchBoyPage implements OnInit {

  boystreamers: any; // 스트리머 목록을 초기화 해줍니다.
  ...
  search(ev) {
    ...
    this.boystreamers= _.filter(this.wsService.boystreamers); // 검색기능은 필터함수를 이용해서 구현했습니다. lodash를 사용했는데 이는 자바스크립트 라이브러리입니다.

    if (searchText != '') { // user가 뭔가를 입력했을 때 해당 입력과 일치하는 스트리머가 있다면 스트리머스에 넣어주는 역할을 하고있는 함수입니다.
      this.boystreamers  =this.boystreamers.filter((boystreamer) => {
        return (boystreamer.Name.toLowerCase()
                .indexOf(searchText.toLowerCase()) > -1);
      });
    }

    else { // 검색을 아무것도 하지 않았을때는 스트리머 목록을 화면에 표시하지 않기 위해서 null값으로 주었습니다.
      this.boystreamers = null;
    }
