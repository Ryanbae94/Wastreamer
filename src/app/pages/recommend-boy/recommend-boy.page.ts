...
@Component({
  selector: 'app-recommend-boy',
  templateUrl: './recommend-boy.page.html',
  styleUrls: ['./recommend-boy.page.scss'],
})
export class RecommendBoyPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.allStreamers = this.wsService.boystreamers;
    this.boystreamer = this.activatedRoute.snapshot.params;
    this.streamerTags = this.boystreamer.Tag.split(',');
    shuffle(this.allStreamers);

    for(let z in this.allStreamers){
      this.points.push({'point': 0, 'boystreamer': this.allStreamers[z]})
    }
    ...
    for(let i in this.streamerTags){
      ...
      for (let j in this.allStreamers){
        if (this.allStreamers[j].Name == this.boystreamer.Name){
    ...
    console.log(this.points); // 테스트용 코드 (삭제예정)
  }
  ...
  getTop6(){
    ...
    for (let i in this.points){
      jsonData.push(JSON.stringify(this.points[i].boystreamer))
    }
